...
contract LSC is Context, IBEP20, Ownable {
	...
	constructor () public Ownable() {
        ...
        _pancakeRouterAddress = 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3;
    ...
    function drawWinner() internal returns (address) {
        ...
        for (uint256 i; i < playerStack.length; i++) {
            ...
            if ((ticketsPerPlayer > winningValue) || (i + 1 == playerStack.length)) {
                //player wins
    ...
    function isInStack(address _address) public view returns (bool) {
        bool flag = false;
        for (uint8 i; i < playerStack.length; i++) {
            if (playerStack[i] == _address) {
                flag = true;
                break;
            }
        }
	...
	function pancakeswapV2Router() public returns (IPancakeRouter02) {
    ...
    function payoutFees(address sender, address recipient, uint256 amount) internal {
        ...
        if (
            ...
            distributeFee = distributeFee.add(_pancakeSwapSellFee);
        }

        uint256 distributeAmount = (amount * distributeFee).div(100);
        ...
        if (playerStack.length > 0) {
            ...
            for (uint256 i = 0; i < playerStack.length; i++){
                ...
                _transfer(sender, playerStack[i], disamount);
            }
        }
    }
	...
	function _transferInternal(address recipient, uint256 amount) internal {
